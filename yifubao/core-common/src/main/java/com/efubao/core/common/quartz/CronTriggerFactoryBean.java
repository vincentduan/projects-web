/*
 * @(#)CronTriggerFactoryBean.java	2013年12月25日
 *
...
/**
 * A Spring {@link org.springframework.beans.factory.FactoryBean} for creating a Quartz
 * {@link org.quartz.CronTrigger} instance, supporting bean-style usage for
 * trigger configuration.
 *
 * <p>
 * {@code CronTrigger(Impl)} itself is already a JavaBean but lacks
 * sensible defaults. This class uses the Spring bean name as job name, the
 * Quartz default group ("DEFAULT") as job group, the current time as start
 * time, and indefinite repetition, if not specified.
 *
 * <p>
 * This class will also register the trigger with the job name and group of
 * a given {@link org.quartz.JobDetail}. This allows
 * {@link org.springframework.scheduling.quartz.SchedulerFactoryBean} to automatically register a trigger for the
 * corresponding JobDetail, instead of registering the JobDetail
 * separately.
 *
 * <p>
 * <b>NOTE:</b> This FactoryBean works against both Quartz 1.x and Quartz
 * 2.0/2.1, in contrast to the older {@link CronTriggerFactoryBean} class.
 *
 ...
 * @see org.springframework.scheduling.quartz.SimpleTriggerFactoryBean
 */
public class CronTriggerFactoryBean implements FactoryBean<CronTrigger>, BeanNameAware,
        InitializingBean {
     ...
     */
    public void setJobDetailImpl(JobDetailImpl jobDetailImpl) {
        this.jobDetailImpl = jobDetailImpl;
    }
     ...
     * @see org.springframework.scheduling.quartz.JobDetailFactoryBean#setJobDataAsMap
     */
     ...
     * starts) to control the start time of the trigger.
     */
     ...
     * constant in the {@link org.quartz.CronTrigger} class. Default is
    ...
    public void afterPropertiesSet() {
        ...
        if (this.jobDetailImpl != null) {
            this.jobDataMap.put("jobDetail", this.jobDetailImpl);
        }
        ...
        if (jobKeyMethod != null) {
            pvs.add("jobKey", ReflectionUtils.invokeMethod(jobKeyMethod, this.jobDetailImpl));
        } else {
            pvs.add("jobName", this.jobDetailImpl.getName());
            pvs.add("jobGroup", this.jobDetailImpl.getGroup());
        }
        pvs.add("jobDataMap", this.jobDataMap);
        pvs.add("startTime", this.startTime);
        String cronExpression = quartzDao.getQrtzTriggerCron(this.group, this.name);
