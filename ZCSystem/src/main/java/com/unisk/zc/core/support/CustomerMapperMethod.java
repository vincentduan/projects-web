...
/**
 * 扩展Mybatis查询功能，支持返回值是com.unisk.is.core.support.Page<E>对象的分页查询
 * 
 * @Description:
 * @author shijingbang
 * @Date 2016年1月8日
 */
public class CustomerMapperMethod {
	...
	public CustomerMapperMethod(Class<?> declaringInterface, Method method, SqlSession sqlSession) {
	...
	private <E> Object executeForMany(Object[] args) {
		...
		if (!method.getReturnType().isAssignableFrom(result.getClass())) {
			if (method.getReturnType().isArray()) {
				...
				return convertToPage(result, args);
			} else {
	...
	public Page convertToPage(List list, Object[] args) {
		Object obj = getParam(args);
		Page<?> page = null;
		if (BaseQuery.class.isAssignableFrom(obj.getClass())) {
			BaseQuery<?> bq = (BaseQuery<?>) obj;
			page = bq.getPage();
			page.setData(list);
		}
		return page;
	...
	private void setupMethodSignature() {
		...
		if (objectFactory.isCollection(method.getReturnType()) || method.getReturnType().isArray() || Page.class.isAssignableFrom(method.getReturnType())) {
			returnsMany = true;
		}
